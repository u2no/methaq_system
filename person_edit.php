...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($full_name) || empty($mobile_number)) {
        ...
        try {
            ...
            /* التحقق من عدم تكرار رقم الجوال عند شخص آخر */
            ...
            $checkPhone->execute([$mobile_number, $person_id]);
            ...
            if ($checkPhone->fetch()) {
                ...
                header('Location: person_edit.php?id=' . $person_id . '&saved=1');
                exit;
...
if (isset($_GET['saved']) && $_GET['saved'] == '1') {
    $success = 'تم تحديث بيانات الشخص بنجاح';
}
...
<main class="content">
    ...
    <div class="white-box">
        ...
        <form method="POST" action="person_edit.php" id="personEditForm" novalidate>

            <input type="hidden" name="id" value="<?php echo htmlspecialchars($person_id, ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <div class="form-group">
                ...
                <select name="status" id="status">
                    ...
                    <option value="نشط" <?php echo $status === 'نشط' ? 'selected' : ''; ?>>
                        نشط
                    </option>

                    <option value="غير نشط" <?php echo $status === 'غير نشط' ? 'selected' : ''; ?>>
                        غير نشط
                    </option>
                ...
                <div class="field-hint">
                    الشخص "غير نشط" لن يظهر ضمن قائمة المستلمين المتاحين عند تسليم مركبة جديدة
                </div>
            ...
            <div class="buttons">
                ...
                <button type="button" class="cancel-btn" onclick="window.location.href='person_detail.php?id=<?php echo urlencode($person_id); ?>'">
                    ✕ إلغاء
                </button>
...
document.getElementById('personEditForm').addEventListener('submit', validatePersonForm);
